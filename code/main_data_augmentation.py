...
parser.add_argument(
    "dataset",
    type=str,
    help="name of the dataset. Must correspond to a valid value that matches names of files in node2vec folder",
)
parser.add_argument(
    "p",
    type=str,
    help="p parameter of node2vec. Must correspond to a valid value that matches names of files in node2vec folder",
)
parser.add_argument(
    "q",
    type=str,
    help="q parameter of node2vec. Must correspond to a valid value that matches names of files in node2vec folder",
)
parser.add_argument(
    "definition",
    type=int,
    help="definition. E.g., 14 for 14:1. Must correspond to a valid value that matches names of files in node2vec folder",
)
parser.add_argument(
    "n_channels",
    type=int,
    help="number of channels that we will be passed to the network. Must not exceed half the depth of the tensors in node2vec folder",
)
parser.add_argument(
    "n_bootstrap",
    type=float,
    help="augmentation ratio. Must be strictly between 0 and 1",
)
...
my_optimizer = "adam"
params = {
    "bandwidth": np.logspace(-1, -0.5, 10)
}  # for the kernel bandwidth grid search

# command line example:
# python main_data_augmentation.py /home/antoine/Desktop/graph_2D_CNN/datasets/ imdb_action_romance 1 1 14 5 0.1
# =============================================================================
...
def get_hist_node2vec(emb, d, my_min, my_max, definition):
    ...
    img_dim = int(
        np.arange(
            my_min,
            my_max + 0.05,
            (my_max + 0.05 - my_min) / float(definition * (my_max + 0.05 - my_min)),
        ).shape[0]
        - 1
    )
    my_bins = np.linspace(
        my_min, my_max, img_dim
    )  # to have middle bin centered on zero
    Hs = []
    for i in range(0, d, 2):
        H, xedges, yedges = np.histogram2d(
            x=emb[:, i], y=emb[:, i + 1], bins=my_bins, normed=False
        )
        Hs.append(H)
    Hs = np.array(Hs)
    return Hs
...
def smoothed_bootstrap(my_array, params):
    ...
    my_bws = np.apply_along_axis(
        get_bw_cv, 0, my_array[: min(100, my_array.shape[0]), :], params
    )

    all_new_coords = []
    for jj in range(
        int(np.random.normal(my_array.shape[0], scale=my_array.shape[0] / 5))
    ):
        rand_row_idx = random.randint(
            0, my_array.shape[0] - 1
        )  # select a row index (i.e., a node) at random
        my_coords = my_array[rand_row_idx, :].tolist()
        new_coords = [0] * len(my_coords)
        for kk in range(len(new_coords)):  # for each dim
            new_coords[kk] = my_means[kk] + (
                my_coords[kk]
                - my_means[kk]
                + np.random.normal(0, scale=(my_bws[kk]) ** 0.5)
            ) / ((1 + my_bws[kk] / my_vars[kk]) ** (0.5))
...
def main():
    ...
    smoothed_bootstrap_partial = partial(
        smoothed_bootstrap, params=params
    )  # for parallelization
    n_jobs = cpu_count()
    ...
    for i in range(n_folds):
        ...
        print(
            "* generating one bootstrapped sample for every",
            inverse_n_b,
            "training obs *",
        )

        idx_for_boot = random.sample(
            range(len(raw_emb_train)), int(round(n_bootstrap * len(raw_emb_train)))
        )
        ...
        new_raw_emb_train = pool.map(
            smoothed_bootstrap_partial,
            [elt for idx, elt in enumerate(raw_emb_train) if idx in idx_for_boot],
        )
        pool.close()

        print(
            len(idx_for_boot),
            "bootstrap samples created in ",
            round(time.time() - tt, 4),
        )
        ...
        shuffled_idxs = random.sample(
            range(len(new_raw_emb_train)), int(len(new_raw_emb_train))
        )  # sample w/o replct
